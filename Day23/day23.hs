...
checkPoint grid (x, y) =
    [(x, y) | not (outOfBounds grid (x, y) || grid !! x !! y == '#')]
...
findAllPaths compressedGraph currPoint endPoint currDistance
  | currPoint == endPoint = [currDistance]
  | Map.member (currPoint, R) compressedGraph =
...
changeFormat directedGraph =
    ...
    ) . (fst . fst)) (Map.toList directedGraph))
...
getUndirectedGraph directedGraph =
    ...
    let backwardsGraph = Map.fromList (map ((\v0 -> -- (Point, [(Point, Int)])
            let valuesToV0 = filter (elem v0 . map fst . snd) (Map.toList forwardGraph) in --[(Point, [(Point, Int)])]
            let valueToDistance = map (\v -> let dist = head (filter (\y -> v0 == fst y) (snd v)) in (fst v, snd dist)) valuesToV0 in (v0, valueToDistance)) . fst) (Map.toList forwardGraph)) in
...
findAllPathsUndirected undirectedGraph currPoint endPoint seenPoints currDistance
  | currPoint == endPoint = [currDistance]
  | currPoint `elem` seenPoints = []
  | otherwise = applyToEach (undirectedGraph Map.! currPoint) undirectedGraph endPoint (currPoint:seenPoints) currDistance
