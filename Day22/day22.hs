...
main = do
    ...
    print (length (filter (\x -> x) (map (safeToDisintegrate supports) [1..length(sortedBricks)])))

    -- Part 2
    print (sum(map (chainReaction supports []) (map createQueue [1..length(sortedBricks)])))
...
updateHighest pts newHighest 
    | and $ map (== snd (head pts)) (tail (map snd pts)) = zip (map fst pts) (repeat newHighest)
    | otherwise = 
        let minZ = minimum (map snd pts) in
            map (\pt -> (fst pt, newHighest - minZ + snd pt)) pts -- If the z's are different
...
safeToDisintegrate supports val = length (filter (== 0) (map (length . filter (/= val)) supports)) == 0
...
chainReaction supports alreadyChecked q = 
    if isQueueEmpty q then length(filter (== []) supports)
        ...
        let newSupports = map (filter (/= val)) supports in 
            let newIndices = map fst (filter (\x -> snd x == []) (zip [1..] newSupports)) in
            chainReaction newSupports (newIndices ++ alreadyChecked) (pushAll updatedQ (filter (\x -> not (elem x alreadyChecked)) newIndices))
