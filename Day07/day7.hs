...
getHandResultP2 hand bet = do
    ...
    let sortedFreq = sortBy (flip compare `on` sec) (fst freqNoJs) -- Sort from most to least frequent
...
removeJsAndGetNumber (x:xs) = let js = removeJsAndGetNumber xs in Data.Bifunctor.first (x :) js
