...
main = do
    ...
    putStrLn (show(sum(map getLineValue linesOfReflection)))

    -- Part 2
    let smudgedLinesOfReflection = map (getSmudgedLinesOfReflection) parsedLines
    putStrLn (show(sum(map getLineValue smudgedLinesOfReflection)))
...
getListEquality xs ys = zipWith (zipWith (==)) xs ys